...
Resume начинает на 5 секунд раньше сохранённой позиции. Любая ненулевая позиция является
точной отметкой; процент, длительность и близость к концу не участвуют в решении. Старую
позицию запрещает только фактический end-сигнал Media3 (`playbackEnded=true`). История
группируется по content ID, но хранит записи отдельных эпизодов.
...
`PlaybackCheckpoint` раздельно несёт explicit `playbackEnded` и `unitActivated`. Обычный
periodic/lifecycle/close checkpoint с position 0 игнорируется и не может затереть прежнее
ненулевое время. Только осознанный переход на другую серию создаёт `unitActivated=true` и
сохраняет новую активную S/E@0 до первого progress tick. При natural end сначала записывается
завершение предыдущей, затем такая активация следующей серии.
...
checkpoint с позицией, explicit episodic activation с position 0 либо completed episode как
fallback-anchor. Реальный completed checkpoint не откатывает пользователя к более старой
незавершённой серии: штатно более новая activation следующей серии становится видимой меткой.
Если activation отсутствовала, Details показывает `Продолжить после SxxExx`, а fresh plan
использует completed anchor для поиска successor. У финальной серии запуск остаётся с начала
той же серии, но пользователь всё равно видит последнюю подтверждённую S/E.
...
### Когда отметка меняется или действительно может исчезнуть

- Ненулевая отметка той же unit обновляется только более новым ненулевым checkpoint либо
  explicit `playbackEnded=true`; обычный position 0 её больше не заменяет.
- Осознанный переход на другую серию делает новой content-level отметкой выбранную S/E@0.
  Старые per-episode записи остаются в DataStore, но одна карточка Истории показывает newest.
- Удаление материала из Истории, очистка Истории, очистка данных приложения или uninstall
  удаляют локальные отметки. Account sync их не восстанавливает.
- Если сохранённая S/E временно отсутствует в fresh provider plan, позиция остаётся в store,
  но не переносится на другую серию; при возвращении той же coordinate она снова пригодна.
- Повреждённая строка codec пропускается отдельно, не стирая остальные записи. Ошибка записи
  DataStore журналируется; без данных с конкретного устройства нельзя задним числом отличить
  её от принудительного завершения процесса или очистки app data.
