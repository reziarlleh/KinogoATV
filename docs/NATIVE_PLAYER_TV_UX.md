...
`WatchProgress` is the source of truth: `PlaybackSelection` + position + duration + monotonic updated time + explicit ended flag, never a signed media URL. Save on the existing 10-second cadence and on state boundaries. Callback order is serialized; a callback from a replaced Media3 generation is ignored, and an older durable write cannot overwrite the newer in-memory checkpoint. Resume rewinds 5 seconds. Every positive unfinished position is exact; only explicit `playbackEnded=true` suppresses it. A regular zero-position lifecycle/close callback is ignored, while explicit `unitActivated=true` stores a deliberately selected episode at zero before the first cadence tick. A completed episode remains a visible successor anchor (`Continue after SxxExx`) and never falls back to an older unfinished episode. Store history per episode, newest first, and allow content-level Delete/Clear from History.
